...
ETHERNET_HDR_SIZE = 14  # Size of an Ethernet frame header.
...
def process_Ethernet_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes) -> None:
    ...
    #NOTE: Implementar aquí el código que procesa una trama Ethernet en recepción
    if len(data) < ETHERNET_HDR_SIZE:
        return          # The frame is not long enough to be processed.
    
    # Check whether the frame was directed to this MAC address.
    dstMac = data[0:6]
    if dstMac != broadcastAddr and dstMac != macAddress:
        return
    ...
    callback = EthernetProtocols.get(etherType)
    if callback is None:
        return
    callback(us, header, payload, srcMac)
...
def registerEthCallback(callback_func: Callable[[ctypes.c_void_p,pcap_pkthdr,bytes],None], ethertype:int) -> None:
    '''
        ...
        Descripción: Esta función recibirá el nombre de una función y su valor de ethertype asociado y añadirá en la tabla 
            ...
            asocaida al Ethertype ARP_ETHERTYPE. 
        Argumentos:
...
def startEthernetLevel(interface:str) -> int:
    ...
    handle = None
    #logging.debug('Función no implementada')
    #NOTE: implementar aquí la inicialización de la interfaz y de las variables globales
    if levelInitialized:
        return -1                       # Level already initialized.
    macAddress = getHwAddr(interface)   # Get MAC address.

    handle = pcap_open_live(interface, ETH_FRAME_MAX, PROMISC, TO_MS, bytearray())  # Open handle.
    if not handle:
        return -1                       # Error while opening handle.
    levelInitialized = True             # Done successfully.
...
def stopEthernetLevel()->int:
    ...
    if not levelInitialized:
        return -1       # If the level has not be initialized, return.

    # Stop the thread.
    recvThread.stop()
    recvThread.join()
    recvThread = None   # Reset the Thread.

    pcap_close(handle)  # Close the opened handle.
    handle = None       # Reset the handle.

    macAddress = None   # Reset the MAC address.

    levelInitialized = False    # The Ethernet level is no longer initialized.
    return 0

def __check_frame_size(data: bytes, length: int):
    """
        It checks and corrects the length of an Ethernet frame.

        Arguments:
            data (bytes): Content of the Ethernet frame.
            length (int): Length of the Ethernet frame.

        Returns:
            (tuple): The frame passed as argument, with bytes initialized
            as 0x00 if its length is below the minimum; and the length
            (with the Ethernet header) of the frame.
            (None): if the frame is too long to be sent via Ethernet. 
    """
    if length + ETHERNET_HDR_SIZE > ETH_FRAME_MAX:
        return None         # Frame would be too long.
    
    # Add 0s until the frame is long enough.
    to_fill = ETH_FRAME_MIN - (ETHERNET_HDR_SIZE + length)
    if to_fill > 0:
        data += bytes(to_fill)
        length = ETH_FRAME_MIN
    else:
        length += ETHERNET_HDR_SIZE
...
def sendEthernetFrame(data:bytes,length:int,etherType:int,dstMac:bytes) -> int:
    ...
    # logging.debug('Función no implementada')
    ret = __check_frame_size(data, length)
    if ret is None:
        return -1   # The frame cannot be sent.
    data, length = ret[0], ret[1]

    # Build Ethernet header.
    ethertype_bytes = struct.pack('!H', etherType)
    frame = dstMac + macAddress + ethertype_bytes + data
    
    # Inject frame.
    if pcap_inject(handle, frame, length) <= 0:
        return -1   # Frame could not be injected successfully.
    return 0