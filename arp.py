...
ARP_ETHERTYPE = 0x0806      # ARP ethertype.
...
def processARPRequest(data:bytes,MAC:bytes)->None:
    '''
        ...
        Retorno: Ninguno
    '''
    #NOTE implementar aquí
    global myIP

    if len(data) < 20:
        return              # Payload too short to be processed.
    
    originMAC = data[:6]
    if originMAC != MAC:
        return              # ARP sent from a different MAC address.
    
    originIP        = struct.unpack('!I', data[6:10])[0]
    destinationIP   = struct.unpack('!I', data[16:20])[0]
    if destinationIP != myIP:
        return              # ARP was for a different IP.
    
    arp_reply = createARPReply(originIP, MAC)
    sendEthernetFrame(arp_reply, len(arp_reply), ARP_ETHERTYPE, MAC)
...
def processARPReply(data:bytes,MAC:bytes)->None:
    ...
    with globalLock:
        if originIP != requestedIP:
            return          # The IP is not the one requested.
        
        # Store the MAC and reset the other values.
        resolvedMAC = originMAC
        awaitingResponse = False
        requestedIP = None

    with cacheLock:
        cache[originIP] = originMAC # Update cache.
...
def createARPRequest(ip:int) -> bytes:
    ...
    global myMAC,myIP
    frame = bytes()
    #NOTE implementar aqui
    
    frame += ARPHeader                      # ARP header.
    frame += struct.pack('!H', 0x0001)      # Opcode: Request.
    frame += myMAC                          # Origin MAC.
    frame += struct.pack('!I', myIP)        # Origin IP.
    frame += (b'\x00' * 6)                  # Unknown MAC address.
    frame += struct.pack('!I', ip)          # IP to be resolved.
...
def createARPReply(IP:int ,MAC:bytes) -> bytes:
    ...
    frame += ARPHeader                      # ARP header.
    frame += struct.pack('!H', 0x0002)      # Opcode: Reply.
    frame += myMAC                          # Origin MAC.
    frame += struct.pack('!I', myIP)        # Origin IP.
    frame += MAC                            # Destination MAC.
    frame += struct.pack('!I', IP)          # Destination IP.
...
def process_arp_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes,srcMac:bytes) -> None:
    '''
        ...
        Descripción: Esta función procesa las tramas ARP. 
            Se ejecutará por cada trama Ethenet que se reciba con Ethertype ARP_ETHERTYPE (si ha sido registrada en initARP). 
    ...
    #NOTE implementar aquí
    if len(data) < 8:
        return                  # Frame too short to process.

    arp_hdr: bytes = data[:ARP_HLEN]
    if arp_hdr != ARPHeader:
        return                  # Not an ARP frame header.
...
def initARP(interface:str) -> int:
    '''
        ...
        Descripción: Esta función construirá inicializará el nivel ARP. Esta función debe realizar, al menos, las siguientes tareas:
            -Registrar la función del callback process_arp_frame con el Ethertype ARP_ETHERTYPE
    ...
    #NOTE implementar aquí
    if arpInitialized:
        return -1       # ARP already initalized.
    
    registerEthCallback(process_arp_frame, ARP_ETHERTYPE)  # Register ARP callback.
    myMAC = getHwAddr(interface)                    # Get MAC address.
    myIP = getIP(interface)                         # Get IP address.

    if ARPResolution(myIP) is not None:             # Check own IP.
        return -1                                   # IP is already taken.

    arpInitialized = True                           # ARP initialized successfully.
    return 0
...
def ARPResolution(ip:int) -> bytes:
    ...
    with cacheLock:
        if ip in cache:
            return cache[ip]    # IP is in cache.

    arp_frame = createARPRequest(ip)
    with globalLock:
        requestedIP = ip
        resolvedMAC = None
    ...
    ret: bytes = None
    for _ in range(3):
        if sendEthernetFrame(arp_frame, len(arp_frame), ARP_ETHERTYPE, broadcastAddr) == -1:
            # Reset global variables.
            with globalLock:
                requestedIP = None
                resolvedMAC = None
                awaitingResponse = False
            return None                     # Could not be sent.

        time.sleep(1)           # Wait for a second untill the ARP Response arrives.
